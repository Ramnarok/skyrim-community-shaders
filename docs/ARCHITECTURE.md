...
- **Tree pose** (test runs 4–6, a close trunk at 11.9% mismatch, with a dark GI band down one side): trees are skinned (`BSTreeNode`, bones sway the branches), and at the Prepass walk their bones' world transforms don't always match the pose the game drew with. Actors do match (0.03%).
  ...
  - The walk now keeps both palettes. Skins whose `frameID` is the newest seen this frame (drawn this frame) are posed from the renderer's matrices (setting "Skinned pose from renderer matrices", on). The others keep their bones: renderer matrices of skins not drawn are stale, measured up to 4,000 units off for actors and 79 for trees. Actors never had a this-frame `frameID` at the Prepass, so they keep their bones. The dump's `scene.skin_pose_vs_renderer_matrices` reports per-kind counts and the largest difference, and `skin_pose_samples` holds raw samples.
  - Result: the trunk went from 11.9% / 2.74% to 0.88% mismatch, leaving a thin sliver on one silhouette edge.
  - The band itself was GI: where the traced trunk sat in front of the drawn one, bounce rays started inside it (full AO, no bounce). GI rays now ignore hits closer than max(2 units, 1% of view distance), the depth metric's tolerance.
