...
*Open: portal-strict lights (dungeons).* Lighting.hlsl applies a portal-strict light only to geometry in the light's rooms (`IsLightIgnored`: `roomFlags` vs the draw's `RoomIndex`). The mask is made before the opaque pass, per pixel, without room information. Counting such lights in the ratio where the raster skips them would darken the lights that do apply. Candidate: a per-pixel room from the rooms' bounds (`BSMultiBoundRoom` → multibound shape, CommonLib to verify), tested against Light Limit Fix's `roomNodes` indices, to replicate `IsLightIgnored` in the trace.
