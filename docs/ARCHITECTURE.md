...
- **Tree pose** (test runs 4–6, a close trunk at 11.9% mismatch, with a dark GI band down one side): trees are skinned (`BSTreeNode`, bones sway the branches), and at the Prepass walk their bones' world transforms don't always match the pose the game drew with. Actors do match (0.03%).
  - `NiSkinInstance::boneMatrices` is the renderer's own per-skin copy of the matrices it uploads. Measured: `numRegisters` = 3, 48 B per skin bone (`allocatedSize` = 48 × bones), rows of a 3×4 in absolute world space, the same form as our palette entries. `frameID` = the frame they were last refreshed (the game frame counter + 4 in every dump).
  - **Trees are always posed from the renderer's matrices** when readable (setting "Skinned pose from renderer matrices", on). The game refreshes a tree's matrices only on some frames but draws with them every frame. Measured per dump: 256–295 tree partitions refreshed this frame, 78–135 one or two frames old, 24–129 three to eight frames old. A first rule that used them only when refreshed this frame made visible trees flip between the drawn pose and their misposed bones from frame to frame. That caused flicker and a mismatch of up to 7.1% on single frames. Trees stay in place, so an off-screen tree's older copy is at most an old sway pose.
  - **Other skins (actors)** keep their bones unless refreshed this frame, which never happened at the Prepass. Their stale copies were measured up to 4,000 units off, and their bones already match (0.03%).
  - The dump's `scene.skin_pose_vs_renderer_matrices` reports per-kind counts, the largest difference and the matrix-age histogram; `skin_pose_samples` holds raw samples.
  - **Result:** depth mismatch 0.34–0.79% at the trunk (from up to 11.9%). A few-pixel sliver remains on one silhouette edge and alternates between traced-nearer and traced-farther across frames, so the traced pose is still sometimes a sway step off the drawn one. **Open:** Jake still sees flicker with GI on (in the debug view with GI off too). The next step is to check whether the depth pre-pass, which the metric and GI positions use, lags the pose the opaque pass draws.
...
- **Interiors:** the game doesn't shadow an interior's directional light (`dirDetailedShadow` only takes the sun's shadow mask when `ShadowDir` is set). GI therefore skips the visibility ray indoors (`GIConstants::Interior`); before this fix the ray always hit the ceiling. Interiors are lit mostly by point lights, which the bounce doesn't sample yet, so RT GI hands interiors back to Screen-Space GI by default (setting "Ray-traced GI in interiors", off). Point-light bounce comes next.
