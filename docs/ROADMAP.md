...
**Where things stand (2026-09-25, late):** M9 (ray-traced direct lighting, below) is under way: phases 1–3 are verified and on (all point lights traced, the game's point-light shadow maps replaced; own shadow channel for the three brightest lights; soft shadows from light size, "Point-light source size" 0.05). Phase 4 (emissives as lights) has its census built and deployed, **waiting for a test run** (F10 near glowing things); next: grow `InstanceData` with the emissive colour and add emission at GI / reflection hits. Open from M9: whether 0.05 source size makes dungeon torch shadows too soft (compare 0.02); hero stability measured only in Bleak Falls. Water (M8) is done except LOD water and the SSR brightness check; CS's `CameraProjUnjittered` holds the inverse projection (CS_NOTES; NRD now gets the real one). Commits are pushed to `origin/skyrim-rt` after each step.
...
4. **Emissive surfaces as lights** (glowing mushrooms, embers, lava): GI and reflection rays that hit a glowing surface add its emission. Census built 2026-09-25, not yet run (ARCHITECTURE §5 "M9")
