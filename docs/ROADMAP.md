...
- Distant LOD in the TLAS. **Land LOD verified 2026-09-24** (run 1: LOD land matches the game's depth far outside the loaded cells, no leak inside, cost within noise); **object LOD verified too (run 2: 225 alpha-tested shapes traced, mismatch outside the loaded cells 80% → 27%, about +0.35 ms per frame)**; tree LOD (not under `lodLandRoot`) is what remains. Built (ARCHITECTURE §3 "M8 distant LOD"): land and object LOD from `TES::lodLandRoot`, clipped in every trace to outside the loaded cells; setting "Distant LOD in the traced scene" (off). Accept when, looking over open land at distant mountains: the dump's LOD census is plausible (land and object shapes traced, few skipped), the depth test outside the loaded cells matches on LOD (mismatch under ~2% on land, tree billboards aside), the depth mismatch *inside* the loaded cells is unchanged with the setting on vs off (no LOD leaking in), and the Prepass hand-off grows by under ~0.3 ms once LOD is resident.
- Water.
