...
**Where things stand (2026-09-25, evening):** M9 phase 5 step 1 (outdoor bounce: sky split from bounce, bounce added as light outdoors) is built and awaiting a sunny-town test run (ARCHITECTURE §5 "M9" phase 5). Phase 4 (emissives) stands with setting "Glow light strength" (Jake: works better above 1; default still 1). Earlier the same day: M9 (ray-traced direct lighting, below) is under way: phases 1–3 are verified and on (all point lights traced, the game's point-light shadow maps replaced; own shadow channel for the three brightest lights; soft shadows from light size, "Point-light source size" 0.05). Phase 4 (emissives as lights): census verified (Blackreach 562 glowing instances, luminance up to 3.9; Riverwood 4, flickering); **step 2 built and run 2026-09-25:** GI and reflection hits on glowing surfaces add their emission × albedo (glow maps per texel from the albedo atlas, `InstanceData` now 64 bytes, setting "Glowing surfaces light the scene", off). In Blackreach it works where it should (ground under the mushrooms; GI +57%, no cost) but moves the final frame only ~1%: bounce light is a small part of that frame, and 263 of 562 emitters there are True PBR in Jake's install (ParallaxGen / Faultier's PBR), which step 2 skips. **Step 3, True PBR emitters, built and run 2026-09-25:** works, but only ~20 of Blackreach's 263 PBR glowing shapes have an emissive texture (the crystals don't). Jake saw no glow in either run: the traced light is there (bounce 5–7× higher inside a glowing cluster) but physically matched emission is a few percent of an emitter's brightness on a dark floor. Added 2026-09-25 to try: "Glow light strength" (0–16, default 1) scales the light glowing surfaces cast, not how they look in reflections (ARCHITECTURE §5 "M9" step 3). Open from M9: whether 0.05 source size makes dungeon torch shadows too soft (compare 0.02); hero stability measured only in Bleak Falls. Water (M8) is done except LOD water and the SSR brightness check; CS's `CameraProjUnjittered` holds the inverse projection (CS_NOTES; NRD now gets the real one). Commits are pushed to `origin/skyrim-rt` after each step.
...
2. **Per-light ratios for the brightest lights** ✅ 2026-09-25 (Riverwood Trader: the heroes are the three big lights, each channel shows only its light's shadows, checked by projecting the lights onto the mask; passes 0.29 ms; hero stability standing still not yet measured). Accept when, in the Riverwood Trader and a torch-lit dungeon room, the three heroes are the obvious big lights, each channel shows only its light's shadows, a surface lit by one light and shadowed from another keeps the first light's colour, heroes don't swap while standing still, and the point-light passes stay under ~0.5 ms. One ratio per pixel is exact for the diffuse sum but not for specular or for a pixel lit by one visible and one occluded light of different colour. An RGBA mask: three channels for the frame's three most important lights (chosen on the CPU, so the trace and `Lighting.hlsl` agree), the fourth for the rest.
...
4. **Emissive surfaces as lights** (glowing mushrooms, embers, lava): GI and reflection rays that hit a glowing surface add its emission. Census verified 2026-09-25; step 2 (emission at GI / reflection hits, glow maps per texel) run 2026-09-25: works, ~1% of the frame in Blackreach; step 3, True PBR emitters, run 2026-09-25: few have an emissive texture; emission is faint at strength 1, knob above 1 undecided (ARCHITECTURE §5 "M9").
5. **Linear radiance composite** (the other half of "full path tracing"): the composite stops scaling the game's gamma-space ambient and adds traced light in linear space. Its own milestone if it grows. **Step 1, outdoor bounce, built 2026-09-25, not yet verified in game:** outdoors the sky is split from the bounce; the sky keeps scaling the game's ambient (own `REBLUR_DIFFUSE_OCCLUSION`, composite t18), and the bounce is added as light, as indoors (ARCHITECTURE §5 "M9" phase 5).
