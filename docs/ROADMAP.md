...
- **Next after water (Jake, 2026-09-25): ray-traced direct lighting.** Today `Lighting.hlsl` computes the sun and point lights and RT only modulates them (shadow masks, ambient ratios). The step towards full path tracing: ReSTIR DI over all lights (the sun and shadow-mapped lights included, emissive surfaces later) evaluated in the trace, and a composite that works in linear radiance instead of scaling the game's gamma-space terms. This crosses CLAUDE.md's "no full replacement of the raster pipeline" non-goal (primary visibility stays raster); update that section when it starts.
