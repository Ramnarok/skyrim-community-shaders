...
namespace RT
{
	...
	bool SunShadows::Fail(std::string a_reason)
	{
		...
		logger::error("[SkyrimRT] Sun shadows setup failed: {}", failureReason);
		return false;
	}
	...
	bool SunShadows::CreatePipelines()
	{
		...
		return create("SunShadowTraceCS.cso", L"SkyrimRT::SunShadowTracePSO", tracePipeline) &&
		       create("SunShadowTemporalCS.cso", L"SkyrimRT::SunShadowTemporalPSO", temporalPipeline) &&
		       create("SunShadowSpatialCS.cso", L"SkyrimRT::SunShadowSpatialPSO", spatialPipeline);
	}
	...
	bool SunShadows::CreateDescriptors()
	{
		...
		// Trace: t1 depth, t2 game shadow mask; u0 raw visibility, u1 geometry.
		...
		uint32_t a_width, uint32_t a_height, ID3D12Resource* a_rasterDepth, ID3D11ComputeShader* a_copyCS, ID3D12Resource* a_alphaAtlas)
	{
		...
		std::string error;
		if (!CreateSharedTexture(d3d11Device, device, width, height, DXGI_FORMAT_R8_UNORM, "SunShadowMask", mask, error) ||
			!CreateSharedTexture(d3d11Device, device, width, height, DXGI_FORMAT_R8G8B8A8_UNORM, "SunShadowView", view, error) ||
			!CreateSharedTexture(d3d11Device, device, width, height, DXGI_FORMAT_R8_UNORM, "GameShadowMaskCopy", gameShadowMask, error))
			return Fail(std::move(error));
		ClearMask();

		if (!CreateTexture(DXGI_FORMAT_R8_UNORM, L"SkyrimRT::SunShadowRaw", rawVisibility) ||
			!CreateTexture(DXGI_FORMAT_R16G16B16A16_FLOAT, L"SkyrimRT::SunShadowGeometry", geometry) ||
			!CreateTexture(DXGI_FORMAT_R16G16B16A16_FLOAT, L"SkyrimRT::SunShadowHistory0", history[0]) ||
			!CreateTexture(DXGI_FORMAT_R16G16B16A16_FLOAT, L"SkyrimRT::SunShadowHistory1", history[1]))
			return false;
		...
		counters->SetName(L"SkyrimRT::SunShadowCounters");
		...
		for (uint32_t i = 0; i < kFramesInFlight; i++) {
			...
			uploads[i]->SetName(L"SkyrimRT::SunShadowUpload");
		...
		logger::info("[SkyrimRT] Sun shadows ready: {}x{} mask", width, height);
		return true;
	}
		...
		bool a_compareShadowMap, bool a_captureDump)
	{
		...
		const float tanHalf = std::tan(std::clamp(a_params.coneHalfAngleDegrees, 0.0f, 10.0f) * std::numbers::pi_v<float> / 180.0f);
		constants->toSun[0] = a_params.toSun[0];
		constants->toSun[1] = a_params.toSun[1];
		constants->toSun[2] = a_params.toSun[2];
		constants->toSun[3] = tanHalf;
		...
		constants->maxHistory = static_cast<float>(std::max(a_params.maxHistory, 1u));
		constants->depthTolerance = kDepthTolerance;
		constants->spatialRadius = std::max(a_params.spatialRadius, 0.0f);
		constants->planeTolerance = kPlaneTolerance;
		constants->compareDistance = kCompareDistance;
		constants->viewMode = a_params.viewMode;
		...
		Barriers(a_list, { TransitionBarrier(counters.get(), D3D12_RESOURCE_STATE_COPY_DEST, kUAV),
							 ...
							 TransitionBarrier(gameShadowMask.resource12.get(), kCommon, kSRV),
		...
		Barriers(a_list, { TransitionBarrier(mask.resource12.get(), kUAV, kCommon),
							 ...
							 TransitionBarrier(gameShadowMask.resource12.get(), kSRV, kCommon),
		...
		if (a_captureDump) {
			...
			if (!dumpReadback && FAILED(CreateBufferResource(device, D3D12_HEAP_TYPE_READBACK, dumpImageBytes * 3, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_FLAG_NONE, dumpReadback.put()))) {
				...
				const Source sources[3] = { { rawVisibility.get(), kSRV }, { mask.resource12.get(), kCommon }, { gameShadowMask.resource12.get(), kCommon } };
	...
	void SunShadows::ReadDumpImages(std::vector<DumpImage>& a_out)
	{
		...
		if (FAILED(dumpReadback->Map(0, nullptr, &mapped)))
			return;
		constexpr const char* kNames[] = { "shadow_raw", "shadow_mask", "game_shadow_mask" };
