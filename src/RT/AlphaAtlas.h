...
namespace RT
{
	...
	/**
	 * @brief M7c alpha test, the start of the v2 texture path (ARCHITECTURE §6): each alpha-tested diffuse texture's
	 * alpha in one kTileSize² tile of an R8 TextureAtlas. Candidates with a tile carry GeometryCandidate::alphaWord; their
	 * instances are traced as non-opaque and the RayQuery passes alpha-test candidate hits against the atlas (MeshData.hlsli).
	 */
	class AlphaAtlas
	{
	public:
		static constexpr uint32_t kTileSize = 512;    // must match MeshData.hlsli
		static constexpr uint32_t kTilesPerRow = 16;  // must match MeshData.hlsli; 8192² R8: 64 MB, 256 tiles
		...
		void ReadDumpImage(std::vector<DumpImage>& a_out) { atlas.ReadDumpImage("alpha_atlas", a_out); }
	...
	/** @brief Static sampler s0 the RayQuery passes sample the atlases with (bilinear, clamp). */
